...
# Start dev server with auto-restart loop to keep it alive
log_step_start "Starting Next.js dev server (with auto-restart)"
echo "[BUN] Starting development server with keepalive..."

while true; do
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting Next.js dev server..."
        npx next dev -p 3000 2>&1 | tee "$PROJECT_DIR/dev.log"
        EXIT_CODE=$?
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Dev server exited (code=$EXIT_CODE), restarting in 2s..."
        sleep 2
done &
...
echo "Next.js dev server is running with auto-restart (PID: $DEV_PID)."
echo "Dev script exiting - server will continue running in background."
